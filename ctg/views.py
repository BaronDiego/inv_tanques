...
def exportar_excel_tanques_ctg(request):
    ...
    for ct in list_idis:
        if CalculoCtg.objects.filter(tanque_id=ct).exists():
            qs_std = CalculoCtg.objects.filter(tanque_id=ct).first()
            qs_std_a.append(qs_std)
    
    for ct in list_idis:
        if CalculoApiCtg.objects.filter(tanque_id=ct).exists():
            qs_api = CalculoApiCtg.objects.filter(tanque_id=ct).first()
    ...
    strToday = today.strftime("%Y%m%d")
    sheet = excel.pe.Sheet(export)
    return excel.make_response(sheet, "xlsx", file_name="EstadoPlantaCtg"+"_"+strToday+".xlsx")
...
def detalle_ocupacion_tk_api_ctg(request, id):
    ...
    lote = LoteApiCtg.objects.filter(id=calculo_lote).values()
    try:
        lote_producto = lote[0]['producto']
        lote_refencia = lote[0]['referencia']
        masa_tk = calculo_tk[0]['masa']
...
@login_required(login_url='login')
def enviar_data_erp_api_ctg(request, id):
    calculo = CalculoApiCtg.objects.filter(tanque_id=id).order_by('-creado')[:2]
    if calculo == "" or calculo == 0:
        calculo = 0
    calculo_tk = CalculoApiCtg.objects.filter(tanque_id=id).order_by('-creado').values()[:1]
