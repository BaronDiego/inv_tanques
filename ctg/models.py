...
class TanqueCtg(ClaseBase):
    bodega= models.CharField(max_length=10, unique=True, null=True, blank=True)
    tag = models.CharField(max_length=50, unique=True)
    terminal = models.CharField(max_length=20, default="CARTAGENA")
...
class AforoTanqueCtg(models.Model):
...
class LoteApiCtg(ClaseBase):
    ...
    def save(self):
        self.referencia = self.referencia.upper()
        self.producto = self.producto.upper()
        super(LoteApiCtg, self).save()


class CalculoApiCtg(ClaseBase):
    tanque = models.ForeignKey(TanqueCtg, on_delete=models.CASCADE)
    lote_api = models.ForeignKey(LoteApiCtg, on_delete=models.CASCADE, related_name='lote_api')
